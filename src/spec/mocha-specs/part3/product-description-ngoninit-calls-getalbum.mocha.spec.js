...
describe("ProductDescription", function () {
  it(`should call the ProductService's getAlbum() method from ngOnInit() @product-description-ngoninit-calls-getalbum`, function () {
    ...
    try {
      file = fs
        .readFileSync(
          __dirname +
            "/../../../app/product-description/product-description.component.ts"
        )
        .toString();
    } catch (e) {
    ...
    assert(
      Array.isArray(match),
      "The ProductDescription `ngOnInit()` method body doesn't contain anything."
    );
    ...
    if (callToGetAlbum.includes("subscribe")) {
      ...
      //   "The call to `getAlbum()` in ProductDescription's `ngOnInit()` method body isn't subscribing to the response and assigning it to `this.albumInfo`."
      // );
    } else {
      let re2 = /this\._productService\s*\.\s*getAlbum\(1\)/;
      assert(
        match[0].match(re2),
        "The ProductDescription `ngOnInit()` method body isn't making the correct call to the ProductService's `getAlbum` method."
      );
    }
  });
  ...
  it(`should define a class property named albumInfo @product-description-ngoninit-calls-getalbum`, function () {
    ...
    assert(
      match[1] == "albumInfo",
      "The ProductDescription doesn't have a class property named `albumInfo`."
    );
  });
});
