...
describe("ProductTracklisting", function () {
  it(`should call the ProductService's getAlbum() method from ngOnInit() @product-tracklisting-ngoninit-calls-getalbum`, function () {
    ...
    try {
      file = fs
        .readFileSync(
          __dirname +
            "/../../../app/product-tracklisting/product-tracklisting.component.ts"
        )
        .toString();
    } catch (e) {
    ...
    assert(
      Array.isArray(match),
      "The ProductTracklisting `ngOnInit()` method body doesn't contain anything."
    );
    ...
    if (callToGetAlbum.includes("subscribe")) {
      ...
      assert(
        false,
        "The ProductTracklisting `ngOnInit()` method body isn't making the correct call to the ProductService's `getAlbum` method."
      );
    }
  });
  ...
  it(`should define a class property named albumInfo @product-tracklisting-ngoninit-calls-getalbum`, function () {
    ...
    assert(
      match[1] == "albumInfo",
      "The ProductTracklisting doesn't have a class property named `albumInfo`."
    );
  });
});
